...
export interface PlatformAdapter {
  ...
  /** Optional: called by audio-tap.ts on every track's decoded-audio speaking edge. */
  onTrackSpeaking?(track: MediaStreamTrack, speaking: boolean): void;
  /** Optional: register a callback for a later async identity upgrade of a track
   * that identify() already resolved (or fell back on) at +track time. */
...
- **`meet.ts`** — **Live-verified, current build (journal `#41`–`#51`): the DOM/CSRC/SSRC/ordering mechanisms originally documented here are all dead** (`#41`–`#46`) — no per-tile `<audio>`/`<video>` element exists at all, CSRC/SSRC don't bridge to a tile, and track/tile cardinality doesn't match 1:1. `identify()` correctly returns `null` and falls back to `speaker-<n>`. A viable mechanism *does* exist (`#49`/`#50`): the `collections` `RTCDataChannel` carries a gzip+protobuf message on every speaking-state transition embedding the participant's stable `spaces/<space>/devices/<device>` id and a start/stop flag, correlated to real speech within tens of milliseconds. **Implemented and live-verified end to end** via the `onTrackSpeaking`/`onIdentify` push interface above: `rtc-hook.ts` parses the channel in production (`identity/meet-collections.ts`), `identity/meet-correlator.ts` pairs a device id's speaking onset to the one live track whose decoded audio also onset within ~200ms, and after `CONFIRM_THRESHOLD` (**1**, journal `#57`–`#58`; started at 3, loosened after live testing showed zero ambiguous matches) confirming turns `MeetAdapter` pushes the upgraded id; `audio-tap.ts` restarts that track's pipeline as a new segment under it (see `audio-tap.ts`'s `handleIdentityUpgrade` for why rename-in-place wasn't used). `identify()` itself is unchanged — still null on this build.
  **No synchronous bootstrap exists.** Meet also sends a larger multi-device "roster" message on `collections` containing SSRC-shaped numbers per device (an RTX/FID pair for video-like entries, a single value for audio-like entries). Investigated whether those numbers could be matched against our own `RTCPeerConnection.getStats()` to identify a participant at `+track` time, before anyone speaks — live-tested (2026-07-19): none of the roster message's numbers appear anywhere in our own receivers' stats (checked every report type). Meet's SFU remaps SSRCs to small local sequential values (`6666`, `6667`, ...) with no numeric relationship to what it reports internally — same conclusion as `#43`'s original SSRC investigation, now confirmed against this message too. Speaking-onset correlation remains the only identity signal; there is no way to resolve identity before the participant's first speaking turn.
  **Wire format, corrected 2026-07-19 (live 3-participant call):** journal `#49` originally documented the speaking-flag path as `1.2.3.10.1`. That's wrong by one nesting level — real messages nest the flag *inside* the same per-device record (field 2, itself nested under field 3) that holds the device id, not as a sibling of that record. The correct path is **`1.2.3.2.10.1`**; the device-id path (`1.2.3.2.6`) was already right. Caught because the production tracer's own "schema changed" self-check (`maybeWarnCollectionsSchema` in `rtc-hook.ts`) fired immediately on real traffic — confirming that safety net works as designed. `identity/meet-collections.ts` also gained `debugDecodeStructure()`, a generic recursive field dump wired into the existing `__earsDebugChannels`-gated tracer, so a future schema drift can be diagnosed from the console instead of an ad-hoc page-injected decoder. `identity/meet-collections.test.ts` now carries real captured wire-byte fixtures (2 devices × start/end) from that session, not just synthetic ones built from the documented schema.
  **Confidence: DOM path — none (verified dead). Collections wire format (device id + speaking flag extraction) — live-verified against real traffic, 2 devices, multiple turns. End-to-end onIdentify upgrade path (correlator confidence threshold, restart-as-new-segment) — still not fully live-confirmed; see journal for what has and hasn't been observed firing.**
...
**Narrow exception (journal `#49`–`#51`, live-verified 2026-07-19):** decoding the Meet `collections` datachannel is permitted **only** for the two specific gzip+protobuf fields documented in `#49` — the `spaces/<space>/devices/<device>` participant id and its paired speaking start/stop flag — and only per the implementation guardrails in `docs/product/browser/prompts/meet-identify-via-collections.md` (defensive/versioned parsing, hard fallback to `speaker-<n>` on any shape mismatch, no dependency on undecoded fields). This does not license decoding `collections` for any other purpose, and does not extend to Zoom's `__reduxStore` or any other private store — those remain fully prohibited.
...
6. No decoding an app's private protobuf/Redux internals for identity — **narrow, bounded exception**: the two Meet `collections` fields documented in journal `#49` (device id, speaking flag), implemented per `docs/product/browser/prompts/meet-identify-via-collections.md` only. Everything else — Zoom's `__reduxStore`, any other Meet `collections` field, any other app's private store — stays fully prohibited.
