...
- **Status (2026-07-18):** popup + service-worker resilience landed and live-verified on Chromium (headless, synthetic harness + stub earsd): capture toggle (`storage.local`, gates via an isolated→MAIN `capture-state` message; OFF supersedes the epoch, mid-call ON re-adopts live tracks), session-gated `chrome.alarms` keepalive + `storage.session` respawn recovery, lazy pcm-port reconnect, and ingest-close of a closing tab's streams. A CDP-forced worker kill mid-call recovered without a tab reload. **Not yet met:** the Firefox half of the exit bar — the `firefox-mv3` build compiles and its manifest is correct (gecko id, `world:"MAIN"` scripts, event-page background — note: an *event page*, not the persistent page the specs assumed; `transport.md` updated), but no live Firefox run has happened, and Firefox lacks `createEncodedStreams` entirely (the Meet path will not capture there as written — Meet-on-Firefox needs its own investigation) while `MediaStreamTrackProcessor` support must be verified on the actual Firefox version. The day-long-soak property is likewise only sampled (multi-minute synthetic soak, forced-respawn recovery), not proven for a full day or against real Meet/Zoom on either browser.
