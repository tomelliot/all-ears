...
- **Mic / device:** `AVAudioEngine` input, bound to a chosen input device rather than blindly following the system default. Selection order: an explicitly configured `device_uid` when present, otherwise the **built-in mic**, otherwise the system default. Preferring the built-in mic by default keeps a connected Bluetooth headset out of the capture path — opening a Bluetooth *input* forces the device off A2DP onto the hands-free profile (mono, 8–16 kHz) and collapses its playback quality for as long as the daemon holds the mic open. The far end of a call is still captured losslessly by the system/app process tap, and the built-in mic hears the local speaker while they wear headphones, so the Bluetooth mic is never needed. Selection is best-effort: if nothing matches or binding fails, capture falls back to the default input. A user who genuinely wants the Bluetooth mic names it explicitly via `device_uid`.
