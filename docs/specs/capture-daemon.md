...
- **Mic / device:** `AVAudioEngine` input on the system default input device. **Known limitation:** when that default is a Bluetooth headset, holding its input open forces the device off A2DP onto the hands-free profile (mono, 8–16 kHz), collapsing its playback quality for as long as the daemon runs. The intended fix — prefer the built-in mic (an explicit `device_uid` otherwise), since the far end of a call is captured losslessly by the system/app process tap and the built-in mic hears the local speaker anyway — is **not yet active**: binding the input node to a chosen device via `AUAudioUnit.setDeviceID` crashed AVFoundation (`AVAudioIOUnit::IOUnitPropertyListener` use-after-free racing the route-change/stall rebuild). The device-selection policy (`InputDeviceSelection`) and enumeration (`CoreAudioInputDeviceEnumerator`) are in place and log the intended choice, but the bind is disabled pending a crash-safe HAL-level selection path.
