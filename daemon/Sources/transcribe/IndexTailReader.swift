...
/// carried over and completed by a later call, so a torn read never
/// mis-parses. Malformed complete lines are reported and skipped, matching
/// ``IndexLog``'s tolerance; unlike `IndexLog.parse`, events are returned in
/// *append order* (a live tail wants arrival order, and the writer already
/// appends chronologically).
struct IndexTailReader {
  ...
  mutating func readNewEvents(onMalformed: (String) -> Void) -> [IndexEvent] {
    ...
    while let newlineIndex = carry.firstIndex(of: 0x0A) {
      let lineBytes = Array(carry[..<newlineIndex])
      carry.removeSubrange(...newlineIndex)
