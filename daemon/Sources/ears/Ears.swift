...
/// `docs/product/specs/control-protocol.md`.
///
...
struct SourcesAddCommand: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "add",
    abstract:
      "Add a source at runtime (currently rejected: Phase 4 seam, see docs/product/specs/capture-daemon.md)."
  )
...
/// browser calls (`docs/product/specs/control-protocol.md`'s "Meeting").
...
struct WatchCommand: AsyncParsableCommand {
  ...
  @Option(
    ...
    help: "Comma-separated telemetry kinds to receive: vad,segment,job. State events "
      + "(meeting,session,source) are always delivered.")
  var events: String = ""
